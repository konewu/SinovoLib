...
//        Log.d(TAG, "解密后的内容：" +dataDecode);  //format ： funCode,data
        ...
        if (funCode.equals("00")) {
            return bindPhone(datavalue);
        }

        //绑定后登录的数据处理
        if (funCode.equals("01")) {
            return loginAfterBond(datavalue);
        }

        //创建新用户的数据处理
        if (funCode.equals("02")) {
            return addNewUser(datavalue);
        }

        //修改用户名
        if (funCode.equals("03")) {
            return editUsername(datavalue);
        }

        //添加数据成功返回值处理
        if (funCode.equals("05")) {
            return createdData(datavalue);
        }

        //删除数据返回值处理
        if (funCode.equals("06")) {
            return delData(datavalue);
        }

        //修改密码类型
        if (funCode.equals("07")) {
            return changeCodeType(datavalue);
        }

        //密码验证，即登录
        if (funCode.equals("08")) {
            return loginResult(datavalue);
        }

        //绑定成功后自动创建用户
        if (funCode.equals("09")) {
            return isAutoCreate(datavalue);
        }

        //清空用户、密码、卡、指纹、日志、恢复出厂设置
        if (funCode.equals("0c")) {
            return clearData(datavalue);
        }

        //校准时间
        if (funCode.equals("10")) {
            return calcTime(datavalue);
        }

        //开关门结果
        if (funCode.equals("0a")) {
            return openOrClose(datavalue);
        }

        //修改密码的结果
        if (funCode.equals("0d")) {
            return changeCode(datavalue);
        }

        //查询电量
        if (funCode.equals("0e")) {
            return checkPower(datavalue);
        }

        //查看门锁状态
        if (funCode.equals("0f")) {
            return lockStatus(datavalue);
        }

        //设置锁的蓝牙名称
        if (funCode.equals("11")) {
            return setLockName(datavalue);
        }

        //查询管理员是否存在
        if (funCode.equals("12")) {
            return checkAdmin(datavalue);
        }

        //同步用户数据、绑定手机
        if (funCode.equals("13")) {
            return syncData(datavalue);
        }

        //同步结束
        if (funCode.equals("14")) {
            return syncDataFinish(datavalue);
        }

        //获取数据的最新修改时间
        if (funCode.equals("15")) {
            return getLastFixedTime(datavalue);
        }

        //设置自动锁门时间
        if (funCode.equals("16")) {
            return autolocktime(datavalue);
        }

        //日志同步
        if (funCode.equals("17")) {
            return syncLog(datavalue);
        }

        //日志同步结束
        if (funCode.equals("18")) {
            return syncLogFinish(datavalue);
        }

        //进入升级模式
        if (funCode.equals("19")) {
            return dfu_ota(datavalue);
        }

        //查看锁的固件版本
        if (funCode.equals("1a")) {
            return checkFirmware(datavalue);
        }

        //删除蓝牙绑定
        if (funCode.equals("1b")) {
            return delBoundPhone(datavalue);
        }

        //查看、设置静音模式
        if (funCode.equals("1c")) {
            return muteMode(datavalue);
        }

        //查看、设置基准时间
        if (funCode.equals("1f")) {
            return baseTime(datavalue);
        }

        //禁用启用 分享密码
        if (funCode.equals("20")) {
            return operateSharedCode(datavalue);
        }

        //查询锁端mac地址
        if (funCode.equals("21")) {
            return getLockMAC(datavalue);
        }

        //查看、设置超级用户权限
        if (funCode.equals("23")) {
            return checkSuperUser(datavalue);
        }

        //查询锁是否被冻结，锁死了（连续5次开门失败）
        if (funCode.equals("2b")) {
            return lockIsFrozen(datavalue);
        }

        //查询锁的基本信息,包括 电量、门锁状态、自动锁门时间、音量大小、用户权限、固件版本
        if (funCode.equals("2c")) {
            return getLockBaseInfo(datavalue);
        }
